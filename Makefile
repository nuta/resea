...
include libs/libresea/arch/$(ARCH)/app_$(ARCH).mk
endif
...
KERNEL_CFLAGS += -DKERNEL -Ikernel/include -Ikernel/arch/$(ARCH)/include -I$(BUILD_DIR)/include
...
$(app_objs): CFLAGS := -DAPP_NAME='"$(1)"' $(APP_CFLAGS) \
...
endef
$(foreach app, $(STARTUPS) $(INIT), $(eval $(call app-make-rule,$(app))))
initfs_files += $(foreach app, $(STARTUPS), $(BUILD_DIR)/initfs/startups/$(app).elf)
...
$(lib_objs): CFLAGS := $(APP_CFLAGS) -Ilibs/$(1)/include
endef
...
$(BUILD_DIR)/initfs/startups/%.elf: $(BUILD_DIR)/apps/%.elf
	mkdir -p $(@D)
	cp $< $@

# Startup.
$(BUILD_DIR)/$(INIT).bin: $(BUILD_DIR)/apps/$(INIT).elf Makefile
...
-include $(foreach app, $(STARTUPS) $(INIT), $(BUILD_DIR)/apps/$(app)/*.deps)
