...
static void print_regs(struct interrupt_regs *regs) {
    ...
    WARN("SS  = %p    RSP = %p    RBP = %p", regs->ss, regs->rsp, regs->rbp);
    WARN("RAX = %p    RBX = %p    RCX = %p", regs->rax, regs->rbx, regs->rcx);
    WARN("RDX = %p    RSI = %p    RDI = %p", regs->rdx, regs->rsi, regs->rdi);
    WARN("R8  = %p    R9  = %p    R10 = %p", regs->r8, regs->r9, regs->r10);
    WARN("R11 = %p    R12 = %p    R13 = %p", regs->r11, regs->r12, regs->r13);
    WARN("R14 = %p    R15 = %p    ERR = %p", regs->r14, regs->r15, regs->error);
}
