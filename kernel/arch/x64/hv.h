    ...
    do {                                                                       \
        ...
        switch (err) {                                                         \
            case ERR_INVALID_ARG:                                              \
                ...
                break;                                                         \
            case ERR_ABORTED:                                                  \
                PANIC("%s failed (VM-instruction error code = %d)",            \
                    #expr, asm_vmread(VMCS_INST_ERROR));                       \
                break;                                                         \
        }                                                                      \
    } while (0)
...
#define ASSERT_GUEST(expr)                                                     \
    do {                                                                       \
        if (!(expr)) {                                                         \
            WARN_DBG("[%s] %s:%d: GUEST ASSERTION FAILURE: %s",                \
                     CURRENT->name,                                            \
                     __FILE__,                                                 \
                     __LINE__,                                                 \
                     #expr);                                                   \
