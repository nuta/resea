    ...
    switch (port) {
        ...
        default:
            LIST_FOR_EACH (device, &guest->pci.devices, struct pci_device, next) {
                if (device->port_base <= port && port < device->port_base + device->port_len) {
                    return device->ops->ioport_read(device, port - device->port_base, size);
                }
            }
    }
...
void handle_io_write(struct guest *guest, uint64_t port, size_t size,
    ...
    switch (port) {
        ...
        default:
            LIST_FOR_EACH (device, &guest->pci.devices, struct pci_device, next) {
                if (device->port_base <= port && port < device->port_base + device->port_len) {
                    device->ops->ioport_write(device, port - device->port_base, size, value);
                    return;
                }
            }
    }
