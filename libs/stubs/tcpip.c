...
handle_t tcpip_listen(uint16_t port, int backlog) {
    ...
    error_t err = ipc_call(server, &m, sizeof(m), &m, sizeof(m));
    if (err != OK) {
        return err;
    }
...
error_t tcpip_write(handle_t handle, const void *data, size_t len) {
    ...
    struct message *m = malloc(sizeof(*m) + TCP_DATA_LEN_MAX);
    m->type = TCP_WRITE_MSG;
    m->tcp_write.handle = handle;
    m->tcp_write.len = len;
    memcpy(m->tcp_write.data, data, len);
    return ipc_call(server, m, sizeof(*m) + len, m, sizeof(*m));
}
...
error_t tcpip_read(handle_t handle, void *buf, size_t buf_len) {
    ...
    struct message m;
    struct message *r = malloc(sizeof(*r) + buf_len);
    ASSERT(buf_len <= TCP_DATA_LEN_MAX);

    m.type = TCP_READ_MSG;
    m.tcp_read.handle = handle;
    m.tcp_read.len = buf_len;
    error_t err = ipc_call(server, &m, sizeof(m), r, sizeof(*r) + buf_len);
    if (IS_ERROR(err)) {
        free(r);
        return err;
    }

    memcpy(buf, r->tcp_read_reply.data, MIN(r->tcp_read_reply.len, buf_len));
    free(r);
    return err;
}
...
error_t tcpip_close(handle_t handle) {
    ...
    return ipc_call(server, &m, sizeof(m), &m, sizeof(m));
}
